...
/**
 * The {@link TradfriColor} is used for conversion between color formats. Use
 * the static methods {@link TradfriColor#fromCie(int, int, int)} and
 * {@link TradfriColor#fromHSBType(HSBType)} for construction.
 *
...
public class TradfriColor {
	...
	// Tradfri uses the CIE color space (see
	 ...
	 * @param xyY
	 *            the CIE y value
	 * @return {@link PercentType} with color temperature (0 = coolest, 100 =
	 *         warmest)
	 */
	public static PercentType calculateColorTemperature(final int xyX, final int xyY) {
		final double x = xyX;
		final double y = xyY;
		double value = 0.0;
		if (((x > PRESET_X[1]) && (y > PRESET_Y[1])) && ((x <= PRESET_X[2]) && (y <= PRESET_Y[2]))) {
			...
			// some bulbs send slighty higher x values for this preset (maybe rounding
			// errors?)
	 ...
	 *            y value 0 to 65535
	 * @param xyBrightness
	 *            brightness from 0 to 254
	 * @return {@link TradfriColor} object with converted color spaces
	 */
	public static TradfriColor fromCie(final int xyX, final int xyY, final int xyBrightness) {
		...
		final double x = unnormalize(xyX);
		final double y = unnormalize(xyY);

		// calculate XYZ using xy and brightness
		final double z = 1.0 - x - y;
		final double Y = (brightness / 254.0);
		...
		final int redRounded = (int) Math.round(red * 255.0);
		final int greenRounded = (int) Math.round(green * 255.0);
		final int blueRounded = (int) Math.round(blue * 255.0);

		// construct hsbType from RGB values
		// this hsbType has corrected values for RGB based on the
		// hue/saturation/brightness values
		final HSBType hsbType = constructHsbTypeFromRgbWithBrightnessPercent(redRounded, greenRounded, blueRounded,
				brightness);
		// take RGB values from the HSBType
		final int rgbR = (int) (hsbType.getRed().intValue() * 2.55);
		final int rgbG = (int) (hsbType.getGreen().intValue() * 2.55);
		final int rgbB = (int) (hsbType.getBlue().intValue() * 2.55);

		return new TradfriColor(rgbR, rgbG, rgbB, xyX, xyY, brightness, hsbType);
	}

	/**
	 * Construct from color temperature in percent. 0 (coldest) to 100 (warmest).
	 * Note: The resulting {@link TradfriColor} has only the {@link TradfriColor#xyX
	 * X} and {@link TradfriColor#xyY y} values set!
	 *
	 * @param percentType
	 *            the color temperature in percent
	...
	public static TradfriColor fromColorTemperature(final PercentType percentType) {
		...
		if (percent < 50.0) {
			...
			final double p = (percent - 50) / 50.0;
	 ...
	 */
	private static int normalize(final double value) {
	 ...
	 */
	private static double unnormalize(final int value) {
	 ...
	 * @param xyBrightness
	 *            xy brightness level 0 to 254
	 * @return {@link PercentType} with brightness level (0 = light is off, 1 =
	 *         lowest, 100 = highest)
	 */
	 ...
	 *            RGB blue value 0 to 255
	 * @param xyX
	 *            CIE x value 0 to 65535
	 * @param xyY
	 *            CIE y value 0 to 65535
	 * @param brightness
	 *            xy brightness level 0 to 254
	 * @param hsbType
	 *            {@link HSBType}
