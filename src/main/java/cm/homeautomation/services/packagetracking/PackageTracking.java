...
public class PackageTracking extends BaseService {
	...
	public void internalUpdateTrackingInformation(String[] args) throws ClientProtocolException, IOException, NoSuchAlgorithmException, KeyManagementException {
	...
	private static boolean isPackageTracked(Package openPackage, List<Package> trackedPackages) {
		for (Package trackedPackage : trackedPackages) {
			if (openPackage.getId().getCarrier().equals(trackedPackage.getId().getCarrier())
					&& openPackage.getId().getTrackingNumber().equals(trackedPackage.getId().getTrackingNumber())) {
				return true;
			}
		}
	...
	public List<Package> getAllOpen() {
		...
		List<Package> resultList = em.createQuery("select p from Package p where p.delivered=0", Package.class)
				.getResultList();
		List<Package> newPackageList = new ArrayList<>();
		for (Package singlePackage : resultList) {

			List<PackageHistory> phList = em.createQuery(
					"select p from PackageHistory p where p.id.trackingNumber=:trackingNumber and p.id.carrier=:carrier order by p.id.timestamp desc",
					PackageHistory.class)
