	...
	return Controller.extend("cm.homeautomation.Main", {
		...
		loadGrafanaData: function(mode) {
			...
			if (mode == "room") {
				...
				oFrameContent.setAttribute("src", "http://" + location.hostname + ":3001/d-solo/UZ8CT7Zgk/messwerte?orgId=1&panelId=2&from=now-2d&to=now&var-ROOMID=" + this.selectedRoom);
			} else if (mode = "power") {

				var oFrame = sap.ui.getCore().byId("grafanaData_power");
				var oFrameContent = oFrame.$()[0];
				console.log(oFrameContent);
				oFrameContent.setAttribute("src", "http://" + location.hostname + ":3001/d-solo/uf0FOlZRk/power-1h-intervals?orgId=1?orgId=1&panelId=2&from=now-2d&to=now");
			}
		},
