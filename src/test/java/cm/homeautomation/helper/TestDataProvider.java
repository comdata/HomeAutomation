...
import de.a9d3.testing.testdata.TestDataStatics;
...
public class TestDataProvider extends de.a9d3.testing.testdata.TestDataProvider {
     ...
     *
     * @param customMap The elements in this map will override the keys of the
     *                  default map
     */
    ...
    /**
     * This factory will return a TestDataProvider which uses the provided seeds for
    ...
    /**
     * This factory will return a TestDataProvider which uses the class defaults for
     * generating test data
     *
     ...
     * @param seed                       The seed which should be used to generate
    ...
    public <T> T fill(Class c, String seed, boolean tryComplexConstructorFirst) {
        ...
        if (fun != null) {
            ...
            System.out.println("looking for class " + c.getName());
    ...
    protected <T> T decideWhichKindOfComplexClassIsNeeded(Class c, String seed, boolean complex) {
        if (Collection.class.isAssignableFrom(c)) {
            ...
            // we have an enum, let's select an entry from that
            List<Object> constants = Arrays.asList(c.getEnumConstants());
            ...
            for (Object object : constants) {
                ...
                if (enumMap.containsKey(clazz)) {
                    System.out.println("returning: "+clazz.getName());
                    return (T) Enum.valueOf(clazz, enumMap.get(clazz));
                }
            }
            return null;
        } else { // something else, try to invoke directly via constructor
            System.out.println("Trying complex class");
    ...
    /**
     * This method will invoke the right kind of collection object for your
     * collection class
     *
    ...
    /**
     * This method will initialize the provided class with null pointer for the
     * mutable variables and random values for the immutable variables (can't be
     * null)
     *
     * @param c   Class which should be initialized
     * @param <T> Return type
     * @return Initialized class
     * @throws IllegalAccessException    is thrown if the access to the class,
     *                                   field, method or constructor is not
     *                                   allowed.
     * @throws InvocationTargetException is thrown when the called method throws an
     *                                   exception.
     */
    ...
    /**
     * Adds custom mappings to the internal provider map. Duplicate entries will be
     * overwritten.
     *
    ...
    public <T> T invokeComplexConstructorWithProviderData(String seed, boolean tryComplexConstructorFirst,
            Constructor constructor) throws InstantiationException, IllegalAccessException, InvocationTargetException {
