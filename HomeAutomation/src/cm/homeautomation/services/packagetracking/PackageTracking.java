...
public class PackageTracking {
	...
	private static void mergeTrackedPackage(Package trackedPackage) {
		...
		List<Package> results = (List<Package>)em.createQuery("select p from Package p where p.id.trackingNumber=:trackingNumber and p.id.carrier=:carrier").setParameter("trackingNumber", trackedPackage.getId().getTrackingNumber()).setParameter("carrier", trackedPackage.getId().getCarrier()).getResultList();
